--[[
    FiHon - The Mimic | Nevoirs WindUI shell
    Source logic: script_goc.lua main logic, All.lua UI/ESP shell
    UI base: Nevoirs_Rage_WindUI_CompactFix.lua style
]]

pcall(function()
    local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
    if pg then
        for _, name in ipairs({"FiHon_The_Mimic_UI", "FiHon_LeftToggle_UI", "Nevoirs_Rage_UI", "Nevoirs_Mimic_UI", "MiniIY_UI", "FiHonLoadError"}) do
...
local GG = __env.GG or {}
__env.GG = GG
_G.FiHonTheMimic_Loaded = true
...
GG.P = GG.P or Players
GG.W = GG.W or workspace
GG.H = GG.H or RunService
GG.R = GG.R or ReplicatedStorage
GG.TeleportService = GG.TeleportService or TeleportService
...
GG.ScriptCache.userIdentify = GG.ScriptCache.userIdentify or {
    ...
    unc_infos = { fireproximityprompt = (fireproximityprompt and "Native") or "Lua" }
}
...
GG.LoadingSignal = GG.LoadingSignal or { Fire = function() end }

local IB_NO_VIRTUALIZE = IB_NO_VIRTUALIZE or function(fn) return fn end
local fireproximityprompt = fireproximityprompt or function(prompt, hold)
    if prompt and prompt.InputHoldBegin and prompt.InputHoldEnd then
        pcall(function()
...
local BLUE = Col3.fromRGB(0, 170, 255);
...
local function __ensureDefaults()
    ...
    cfg.B1C4 = cfg.B1C4 or { ESP = {} }
    cfg.B2C1 = cfg.B2C1 or { ESP = {} }
    ...
    cfg.NightmareCircus.Ringmaster = cfg.NightmareCircus.Ringmaster or {}
    cfg.Emote = cfg.Emote or { ["Select Emote"] = "None" }
    cfg.Client = cfg.Client or {}
    cfg.Client.Client = cfg.Client.Client or { FlySpeed = 1 }
    cfg.Client.Client.FlySpeed = cfg.Client.Client.FlySpeed or 1
    cfg.Client.Client["TeleportWalk Speed"] = cfg.Client.Client["TeleportWalk Speed"] or 1
    Configs = cfg
end
__ensureDefaults()

local pir = pairs
local LocalPlayer = GG.P.LocalPlayer
local isMobile = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
local controlModule = nil
pcall(function()
    controlModule = require(LocalPlayer:WaitForChild("PlayerScripts"):WaitForChild("PlayerModule")):GetControls()
end)
controlModule = controlModule or { GetMoveVector = function() return Vector3.zero end }

local function __toast(title, content, duration)
    pcall(function()
        StarterGui:SetCore("SendNotification", {
            Title = tostring(title or "FiHon"),
...
local function __errorGui(title, message)
    warn("[FiHon - The Mimic] " .. tostring(title) .. ": " .. tostring(message))
    __toast(title, message, 8)
    pcall(function()
        local pg = LocalPlayer and LocalPlayer:WaitForChild("PlayerGui", 10)
        if not pg then return end
        local old = pg:FindFirstChild("FiHonLoadError")
        if old then old:Destroy() end
        local gui = Instance.new("ScreenGui")
        gui.Name = "FiHonLoadError"
        gui.ResetOnSpawn = false
        gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
        gui.Parent = pg
        local frame = Instance.new("Frame")
        frame.Size = UDim2.fromOffset(430, 135)
        frame.Position = UDim2.new(0.5, -215, 0.12, 0)
        ...
        label.Text = tostring(title or "FiHon") .. "\n" .. tostring(message or "")
        label.Parent = frame
        task.delay(12, function() if gui and gui.Parent then gui:Destroy() end end)
    end)
end

local function __notify(title, content, icon, duration)
    local ui = GG.ScriptCache and GG.ScriptCache.WindUI
    if ui and ui.Notify then
        local ok = pcall(function()
            ui:Notify({ Title = title or "FiHon", Content = tostring(content or ""), Icon = icon or "info", Duration = duration or 4 })
        end)
        if ok then return end
    end
    __toast(title, content, duration)
end
...
local function __scopeRoot(scope)
    Configs = GG.Configs
    if scope == "Client" then
        Configs.Client = Configs.Client or {}
        Configs.Client.Client = Configs.Client.Client or {}
        return Configs.Client.Client
    elseif scope == "Emote" then
        Configs.Emote = Configs.Emote or { ["Select Emote"] = "None" }
        return Configs.Emote
    else
        Configs[scope] = Configs[scope] or {}
        return Configs[scope]
    end
end
local function __getConfig(scope, path)
    local root = __scopeRoot(scope)
    local parts = __split(path)
    if scope == "Client" and parts[1] == "Client" then table.remove(parts, 1) end
    local cur = root
    for i = 1, #parts do
...
local function __setConfig(scope, path, value)
    local root = __scopeRoot(scope)
    local parts = __split(path)
    if scope == "Client" and parts[1] == "Client" then table.remove(parts, 1) end
...
CorePackage = function()
    return {
        { type = "Group", dats = {{
            Title = "Core",
            Open = true,
            dat = {
                { type = "Button", EN = "Rejoin", EN2 = "Rejoin current server.", Callback = function()
                    TeleportService:Teleport(game.PlaceId, LocalPlayer)
                end },
                { type = "Button", EN = "Destroy UI", EN2 = "Close FiHon UI.", Callback = function()
                    if GG.ScriptCache.Window and GG.ScriptCache.Window.Destroy then GG.ScriptCache.Window:Destroy() end
                end },
            }
        }}},
    }
...
WindLib = function()
    if __env.WindUI then return __env.WindUI end
    local lib, err = loadWindUI()
    if not lib then
        __errorGui("Không tải được WindUI", "Hãy bật HttpGet/GitHub trong executor. Error: " .. tostring(err))
        error("Không tải được WindUI: " .. tostring(err))
    end
    __env.WindUI = lib
    return lib
end
...
IntroLib = { Init = function() end, Tutorial = function() end }

Windy = {}
function Windy:CreateComponent(tab, components, scope)
    if not tab or not components then return end
    local COMPACT_CONTROLS = true

    local function safeParagraph(container, title, desc, icon)
        if not container or not container.Paragraph then return end
        pcall(function()
            container:Paragraph({
                Title = tostring(title or "Section"),
                Desc = (COMPACT_CONTROLS and "") or tostring(desc or ""),
                Image = icon or "list",
                ImageSize = 12,
    ...
    local function safeDivider(container)
        pcall(function() if container and container.Divider then container:Divider() end end)
    end
    local function safeCallback(title, cb, ...)
        if not cb then return end
        local args = {...}
        task.spawn(function()
            local ok, err = pcall(function() return cb(unpack(args)) end)
            if not ok then __notify("Lỗi: " .. tostring(title), tostring(err), "circle-alert", 6) end
        end)
    end
    local function addButton(container, item)
        if not container or not container.Button then return end
        local title = item.EN or item.Title or item.TH1 or "Button"
        local desc = item.EN2 or item.Desc or item.TH2 or ""
        local options = { Title = title, Locked = item.Locked, Callback = function() safeCallback(title, item.Callback) end }
        if not COMPACT_CONTROLS and desc ~= "" then options.Desc = desc end
        local ok, err = pcall(function() container:Button(options) end)
        if not ok then warn("[FiHon UI] Button lỗi " .. tostring(title) .. ": " .. tostring(err)) end
    end
    local function addToggle(container, item)
        if not container or not container.Toggle then return end
        local title = item.EN or item.Title or item.TH1 or "Toggle"
        local desc = item.EN2 or item.Desc or item.TH2 or ""
        local current = false
        if item.Path then
            current = __getConfig(scope, item.Path)
            if current == nil then __setConfig(scope, item.Path, false); current = false end
        elseif item.Value ~= nil then
            current = item.Value and true or false
        end
        local flag = tostring(scope or "FiHon") .. "/" .. tostring(item.Path or title):gsub("%W", "_")
        local options = {
            Title = title,
            Flag = flag,
            Value = current and true or false,
            Locked = item.Locked,
            Callback = function(state)
                state = state and true or false
                if item.Path then __setConfig(scope, item.Path, state) end
                safeCallback(title, item.Callback, state)
            end,
        }
        if not COMPACT_CONTROLS and desc ~= "" then options.Desc = desc end
        local ok, err = pcall(function() container:Toggle(options) end)
        if not ok then warn("[FiHon UI] Toggle lỗi " .. tostring(title) .. ": " .. tostring(err)) end
    end
    local function addSlider(container, item)
        if not container or not container.Slider then return end
        local title = item.EN or item.Title or item.TH1 or "Slider"
        local desc = item.EN2 or item.Desc or item.TH2 or ""
        local val = item.Value or {}
        local minv = tonumber(val.Min) or 0
        local maxv = tonumber(val.Max) or 100
        local default = item.Path and __getConfig(scope, item.Path) or val.Default
        default = tonumber(default) or tonumber(val.Default) or minv
        if item.Path and __getConfig(scope, item.Path) == nil then __setConfig(scope, item.Path, default) end
        local options = {
            Title = title,
            Step = ((maxv - minv) <= 10) and 0.1 or 1,
            IsTextbox = true,
            IsTooltip = false,
            Width = UserInputService.TouchEnabled and 130 or 150,
            Value = { Min = minv, Max = maxv, Default = default },
            Locked = item.Locked,
            Callback = function(value)
                value = tonumber(value) or default
                if item.Path then __setConfig(scope, item.Path, value) end
                safeCallback(title, item.Callback, value)
            end,
        }
        if not COMPACT_CONTROLS and desc ~= "" then options.Desc = desc end
        local ok, err = pcall(function() container:Slider(options) end)
        if not ok then warn("[FiHon UI] Slider lỗi " .. tostring(title) .. ": " .. tostring(err)) end
    end
    local function addDropdown(container, item)
        if not container or not container.Dropdown then return end
        local title = item.EN or item.Title or item.TH1 or "Dropdown"
        local desc = item.EN2 or item.Desc or item.TH2 or ""
        local values = item.Values or {}
        local current = item.Path and __getConfig(scope, item.Path) or item.Value
        if current == nil then current = item.Value or values[1] end
        if item.Path and current ~= nil then __setConfig(scope, item.Path, current) end
        local options = {
            Title = title,
            Values = values,
            Value = current,
            SearchBarEnabled = #values > 6,
            Locked = item.Locked,
            Callback = function(option)
                local value = type(option) == "table" and (option.Title or option.Value or option[1]) or option
                if item.Path then __setConfig(scope, item.Path, value) end
                safeCallback(title, item.Callback, value)
            end,
        }
        if not COMPACT_CONTROLS and desc ~= "" then options.Desc = desc end
        local ok, err = pcall(function() container:Dropdown(options) end)
        if not ok then warn("[FiHon UI] Dropdown lỗi " .. tostring(title) .. ": " .. tostring(err)) end
    end

    local function addElement(container, item)
        if type(item) ~= "table" then return end
        local typ = item.type or (item.dat and "Group") or (item.dats and "Group") or "Button"
        if typ == "Space" then
            safeDivider(container)
            return
        elseif typ == "Divider" then
            safeDivider(container)
            return
        elseif typ == "Group" or item.dat or item.dats then
            local groups = item.dats or { item }
            for _, group in ipairs(groups) do
                if group.Title or item.Title then
                    safeParagraph(container, group.Title or item.Title, group.Desc or item.Desc, "list")
                end
                local list = group.dat or group.dats or {}
                for _, child in ipairs(list) do addElement(container, child) end
            end
            return
        elseif typ == "Button" then
            addButton(container, item)
        elseif typ == "Toggle" then
            addToggle(container, item)
        elseif typ == "Slider" then
            addSlider(container, item)
        elseif typ == "Dropdown" then
            addDropdown(container, item)
        else
            safeParagraph(container, item.EN or item.Title or typ, item.EN2 or item.Desc or "", "info")
        end
    end

    for _, component in ipairs(components) do addElement(tab, component) end
end

CommonF = {}
function CommonF._pos(obj)
...
function CommonF.Tp(primary, cf, waitTime)
    if (not primary) or (typeof(primary) == "Instance" and not primary.Parent) then
        local char = LocalPlayer.Character
        primary = char and (char:FindFirstChild("HumanoidRootPart") or char:FindFirstChildWhichIsA("BasePart"))
    end
    ...
    pcall(function()
        if primary:IsA("Model") then primary:PivotTo(target) else primary.CFrame = target end
    end)
    if waitTime then task.wait(waitTime) end
end
function CommonF.Tween(args)
    if not args or not args.goal then return end
    local primary = args.primary
    if (not primary) or (typeof(primary) == "Instance" and not primary.Parent) then
        local char = LocalPlayer.Character
        primary = char and (char:FindFirstChild("HumanoidRootPart") or char:FindFirstChildWhichIsA("BasePart"))
    end
    ...
    if not info and goal.CFrame then
        local d = (CommonF._pos(primary) - goal.CFrame.Position).Magnitude
        info = TweenInfo.new(math.clamp(d / 120, 0.35, 8), Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
    end
    info = info or TweenInfo.new(1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
    local tw = TweenService:Create(primary, info, goal)
    tw:Play()
    tw.Completed:Wait()
    return tw
end
function CommonF.dist(pos)
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then return math.huge end
    return (root.Position - CommonF._pos(pos)).Magnitude
end
function CommonF.distOf(a, b, range)
    local d = (CommonF._pos(a) - CommonF._pos(b)).Magnitude
    return range and d <= range or d
end
function CommonF.findPrompt(list, predicate)
    for _, v in pairs(list or {}) do
        if typeof(v) == "Instance" and v:IsA("ProximityPrompt") then
            local ok, result = pcall(function()
                if predicate then return predicate(v) end
                return true
            end)
            if ok and result then return v end
        end
    end
end
function CommonF:CKey(key, hold)
    pcall(function()
...
function CommonF.HumanoidEquip(humanoid, tool)
    if humanoid and tool then pcall(function() humanoid:EquipTool(tool) end) end
end
function CommonF.Anchored(part, state)
    if part then pcall(function() part.Anchored = state end) end
end

ClientPackage = {}
local __lightingCache = {
...
function ClientPackage.UpdatePosition(floatEnabled, forceFloat, root)
    if not root then return end
    local enabled = (forceFloat == true) or (floatEnabled == true and forceFloat ~= false)
    local bv = root:FindFirstChild("FiHonFloat")
    if enabled then
        if not bv then
            bv = Instance.new("BodyVelocity")
            bv.Name = "FiHonFloat"
            bv.MaxForce = Vector3.new(0, 9e5, 0)
...
ClientPackage.__NoclipCache = ClientPackage.__NoclipCache or setmetatable({}, { __mode = "k" })
ClientPackage.__NoclipWasOn = ClientPackage.__NoclipWasOn or false
function ClientPackage.Noclip(state, char)
    if not char then return end
    local cache = ClientPackage.__NoclipCache

    if state then
        ClientPackage.__NoclipWasOn = true
    ...
    if ClientPackage.__NoclipWasOn then
        ...
        ClientPackage.__NoclipWasOn = false
    end
end
function ClientPackage.Brightness(state)
    if state then
        ...
        for k, v in pairs(__lightingCache) do pcall(function() Lighting[k] = v end) end
    end
end

PromptPackage = { _running = false, _cache = {} }
function PromptPackage.SwitchUNC() end
function PromptPackage.UpdateState(disabled)
    PromptPackage.Disabled = disabled
    if disabled then
        for prompt, old in pairs(PromptPackage._cache) do
            if prompt and prompt.Parent then pcall(function() prompt.HoldDuration = old end) end
        end
        return
    end
    if PromptPackage._running then return end
    PromptPackage._running = true
    task.spawn(function()
        while PromptPackage._running and not PromptPackage.Disabled do
            local cfg = GG.Configs and GG.Configs.Client and GG.Configs.Client.Client
            if cfg and cfg["Instant Prompt"] then
                for _, prompt in ipairs(workspace:GetDescendants()) do
                    if prompt:IsA("ProximityPrompt") then
                        if PromptPackage._cache[prompt] == nil then PromptPackage._cache[prompt] = prompt.HoldDuration end
...
CoruTask = { Tasks = {}, Running = {}, Intialized = false }
function CoruTask.Init() end
function CoruTask.New(name, fn) CoruTask.Tasks[name] = fn end
function CoruTask.Close(name)
    CoruTask.Running[name] = nil
    error("__FIHON_CLOSE_TASK__" .. tostring(name), 0)
end
function CoruTask.Handle(name)
    if CoruTask.Running[name] then return end
    local fn = CoruTask.Tasks[name]
    if not fn then return end
    CoruTask.Running[name] = true
    task.spawn(function()
        local ok, err = pcall(fn)
        CoruTask.Running[name] = nil
        if not ok and not tostring(err):find("__FIHON_CLOSE_TASK__", 1, true) then
            warn("[FiHon CoruTask] " .. tostring(name) .. ": " .. tostring(err))
        end
    end)
end

ESPF = { _objects = {}, _heartbeat = nil }
local function __espParent()
    local ok, parent = pcall(function() return CoreGui end)
    if ok and parent then return parent end
    return LocalPlayer:WaitForChild("PlayerGui")
end
local function __validInst(obj)
    return typeof(obj) == "Instance" and obj.Parent ~= nil
end
local function __validPart(obj)
    return __validInst(obj) and obj:IsA("BasePart")
end
local function __espAdornee(target)
    if typeof(target) ~= "Instance" then return nil, nil end
    if target:IsA("BasePart") then
        local model = target:FindFirstAncestorWhichIsA("Model")
        if model and model ~= W then
            return model, target
        end
        return target, target
    end
    if target:IsA("Model") then
        local part = target:FindFirstChild("HumanoidRootPart") or target.PrimaryPart or target:FindFirstChild("Head") or target:FindFirstChildWhichIsA("BasePart", true)
        return target, part
    end
    local part = target:FindFirstAncestorWhichIsA("BasePart")
    return part or target, part
end
local function __espKey(target)
...
local __espLabelMap = {
    ["City/Glass"] = "Glass",
    ["City/Enzukai"] = "Enzukai",
    ["Mall/Coins"] = "Coin",
    ["Mall/Talk"] = "Walkie",
    ["Mall/Monsters"] = "Monster",
    ["Draw/Senzai"] = "Senzai",
    ["Draw/Ingredients"] = "Ingredient",
...
local function __espLabel(tag, target)
    local raw = tostring(tag or "ESP")
    local mapped = __espLabelMap[raw]
    if mapped then return mapped end
    local last = raw:match("[^/]+$") or raw
    if typeof(target) == "Instance" then
        local n = target.Name or ""
        if #last > 14 and #n > 0 and #n <= 18 then
            last = n
        end
    end
    last = tostring(last):gsub("^ESP%s*", "")
    if #last > 18 then
        last = last:sub(1, 17) .. "…"
    end
    return last ~= "" and last or "ESP"
end
local function __destroyESPEntry(entry)
    if not entry then return end
    pcall(function() if entry.distConn then entry.distConn:Disconnect() end end)
...
local function __ensureESPHeartbeat()
    if ESPF._heartbeat then return end
    ESPF._heartbeat = H.Heartbeat:Connect(function()
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        local alive = false
        for tag, bucket in pairs(ESPF._objects) do
            if bucket then
                for key, entry in pairs(bucket) do
                    if (not entry) or (not entry.bb) or (not entry.bb.Parent) or (not entry.anchor) or (not entry.anchor.Parent) then
                        __destroyESPEntry(entry)
                        bucket[key] = nil
                    else
                        alive = true
                        if root and entry.distLbl then
                            entry.distLbl.Text = string.format("%.0f st", (root.Position - entry.anchor.Position).Magnitude)
                        end
                    end
                end
            end
        end
        if not alive and ESPF._heartbeat then
            ESPF._heartbeat:Disconnect()
            ESPF._heartbeat = nil
        end
    end)
end
function ESPF.Destroy(tag)
    local bucket = ESPF._objects[tag]
    if bucket then
        for key, entry in pairs(bucket) do
            __destroyESPEntry(entry)
            bucket[key] = nil
    ...
    ESPF._objects[tag] = nil
end
function ESPF.ESP(tag, target, color, size, useModelCharm)
    local adornee, anchor = __espAdornee(target)
    if not __validInst(adornee) or not __validPart(anchor) then return end

    ESPF._objects[tag] = ESPF._objects[tag] or {}
    local key = __espKey(target)
    if ESPF._objects[tag][key] then return end

    local label = __espLabel(tag, target)
    local c = color or Color3.fromRGB(255, 255, 255)
    local charmAdornee = adornee
    if useModelCharm == false and target and target:IsA("BasePart") then
        charmAdornee = target
    end
    ...
    local ok1, hl = pcall(function()
        ...
        h.Name = "FiHon_HL"
        h.Adornee = charmAdornee
        h.FillColor = c
        h.OutlineColor = Color3.new(1, 1, 1)
        h.FillTransparency = 0.52
        h.OutlineTransparency = 0.05
        h.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
        h.Parent = (__validInst(charmAdornee) and charmAdornee) or __espParent()
        return h
    end)
    ...
    local ok2, bb = pcall(function()
        ...
        b.StudsOffset = Vector3.new(0, math.max(2.15, ((size and size.Y) or 3) * 0.50), 0)
        b.AlwaysOnTop = true
        b.LightInfluence = 0
        b.Parent = anchor
        return b
    ...
    nameLbl.Size = UDim2.new(1, 0, 0.58, 0)
    ...
    distLbl.Size = UDim2.new(1, 0, 0.42, 0)
    distLbl.Position = UDim2.new(0, 0, 0.58, 0)
    distLbl.BackgroundTransparency = 1
    distLbl.TextColor3 = Color3.fromRGB(210, 210, 220)
    ...
    ESPF._objects[tag][key] = {
        hl = hl,
        bb = bb,
        distLbl = distLbl,
        adornee = charmAdornee,
        anchor = anchor,
    }
    __ensureESPHeartbeat()
end

TheMimicPaints = TheMimicPaints or {
    Emotes = {
        ["None"] = "rbxassetid://0",
    ...
    Init = function()
        return rawget(GG, "TheMimicPaints")
            or rawget(GG, "TheMimicPaintsData")
            or rawget(__env, "TheMimicPaints")
            or rawget(__env, "TheMimicPaintsData")
            or rawget(_G, "TheMimicPaints")
            or rawget(_G, "TheMimicPaintsData")
            or {}
    end,
}
...
-- End compatibility layer ------------------------------------------------------
        ...
        local Backpack          = selff:FindFirstChild("Backpack") or WaitForChild(selff, "Backpack", 10);
        local PSG               = selff:FindFirstChildOfClass("PlayerGui") or WaitForChild(selff, "PlayerGui", 10);
        local selc              = selff.Character or Instancen("Model");
        local HumSelf           = FindFirstChildOfClass(selc, "Humanoid") or FindFirstChild(selc, "Humanoid");
        ...
        Functions.SetFlyKey = IB_NO_VIRTUALIZE(function(input, down)
            if not ClientCon["Enable Fly"] or not input then return; end;
            local key = input.KeyCode;
            local speed = tonumber(ClientCon.FlySpeed) or 1;
            if key == Enum.KeyCode.W then Control.F = down and speed or 0; end;
            if key == Enum.KeyCode.S then Control.B = down and -speed or 0; end;
            if key == Enum.KeyCode.A then Control.L = down and -speed or 0; end;
            if key == Enum.KeyCode.D then Control.R = down and speed or 0; end;
            if key == Enum.KeyCode.Q then Control.Q = down and -speed or 0; end;
            if key == Enum.KeyCode.E or key == Enum.KeyCode.Space then Control.E = down and speed or 0; end;
        end);
        ...
        Functions.B2C1ESP = function(self, where)
            ESPF.Destroy(where);
            if where == "Rat/Monsters" or where == "Ship/Monsters" then
                if GameAI then
                    for _,v in pairs(GetChildren(GameAI)) do
                        local hrp = FindFirstChild(v, "HumanoidRootPart") or FindFirstChild(v, "Hitbox") or FindFirstChildWhichIsA(v, "BasePart");
                        if hrp then ESPF.ESP(where, hrp, RED, VEC10, nil, nil, false, false); end;
                    end;
                end;
                if GameAI2 then
                    for _,v in pairs(GetChildren(GameAI2)) do
            ...
            elseif where == "Rat/Objects" then
                for _,v in pairs(GetDescendants(W)) do
                    if IsA(v, "ProximityPrompt") and v.Parent and IsA(v.Parent, "BasePart") then
                        local okTarget = false;
                        pcall(function()
                            okTarget = (v.Parent.Name == "Interact" and v.Parent.Parent and v.Parent.Parent.Name == "Book")
                                or (v.Parent.Name == "Cube" and v.Parent.MeshId == "https://assetdelivery.roblox.com/v1/asset/?id=8569135676")
                                or (v.ObjectText == "Old Well")
                                or (v.ObjectText == "Bell")
                                or (v.ActionText == "Exit")
                        end);
                        if okTarget then ESPF.ESP(where, v.Parent, GREEN, VEC2, nil, nil, false, false); end;
                    end;
                end;
                return;
            end;
        ...
        local function __B2C4Notify(title, content, icon, duration)
            if WindUI and WindUI.Notify then
                return pcall(function()
                    WindUI:Notify({
                        Title = title or "B2C4",
                        Content = content or "",
                        Icon = icon or "circle-alert",
        ...
        local function __B2C4FirePrompt(prompt, times)
            if typeof(prompt) ~= "Instance" or not prompt:IsA("ProximityPrompt") then return false end
            times = times or 1
            pcall(function()
                prompt.MaxActivationDistance = math.max(tonumber(prompt.MaxActivationDistance) or 0, 999)
        ...
        local function __B2C4TpPrompt(obj, times, yOffset)
            local prompt = __B2C4GetPrompt(obj)
            local part = __B2C4GetPart(prompt and prompt.Parent or obj) or __B2C4GetPart(obj)
            if not prompt or not part then return false end
            local cf = part.CFrame
            if yOffset then cf = cf + Vec3(0, yOffset, 0) end
            CommonF.Tp(HumRSelf, cf, 0.12)
            return __B2C4FirePrompt(prompt, times or 1)
        end
        local function __B2C4FallbackPaint()
            local data = {}
            local down = true
            for row = 0, 10 do
                local y = -0.45 + (row * 0.09)
                if row % 2 == 0 then
                    tble.insert(data, {-0.45, y, down})
                    tble.insert(data, {0.45, y, down})
                else
                    tble.insert(data, {0.45, y, down})
                    tble.insert(data, {-0.45, y, down})
            ...
            for col = 0, 10 do
                local x = -0.45 + (col * 0.09)
                if col % 2 == 0 then
                    tble.insert(data, {x, -0.45, down})
                    tble.insert(data, {x, 0.45, down})
                else
                    tble.insert(data, {x, 0.45, down})
                    tble.insert(data, {x, -0.45, down})
        ...
        local function __B2C4FindPaintData(assetId)
            assetId = tonumber(assetId)
            if not assetId then return nil end
            local function validData(data)
                return type(data) == "table" and #data > 0 and type(data[1]) == "table"
            end
            local function fromTable(tbl, depth, seen)
                if type(tbl) ~= "table" or depth > 3 then return nil end
                seen = seen or {}
                if seen[tbl] then return nil end
                seen[tbl] = true
                local direct = rawget(tbl, assetId) or rawget(tbl, tostring(assetId)) or rawget(tbl, "rbxassetid://" .. tostring(assetId))
                if validData(direct) then return direct end
                for _, v in pairs(tbl) do
                    if validData(v) then
                        -- keep searching unless this table is directly keyed by a matching asset id
                    elseif type(v) == "table" then
                        local got = fromTable(v, depth + 1, seen)
                        if validData(got) then return got end
                    end
                end
                return nil
            end
            ...
            local sources = {
                Paints,
                rawget(GG, "TheMimicPaints"),
                rawget(GG, "TheMimicPaintsData"),
                rawget(__env, "TheMimicPaints"),
                rawget(__env, "TheMimicPaintsData"),
                rawget(_G, "TheMimicPaints"),
                rawget(_G, "TheMimicPaintsData"),
            }
            for _, src in ipairs(sources) do
                local got = fromTable(src, 0, {})
                if validData(got) then return got end
            end
                ...
                if ok and type(gc) == "table" then
                    for _, obj in ipairs(gc) do
                        if type(obj) == "table" then
                            local got = rawget(obj, assetId) or rawget(obj, tostring(assetId)) or rawget(obj, "rbxassetid://" .. tostring(assetId))
                            if validData(got) then return got end
                        end
                    end
                end
        ...
        local function __B2C4UniversalDraw(remote)
            local sent = 0
            local data = __B2C4FallbackPaint()
            for _, v in ipairs(data) do
                pcall(function() remote:FireServer(v[1], v[2], v[3]) end)
                pcall(function() remote:FireServer(Vector2.new(v[1], v[2]), v[3], true) end)
                pcall(function() remote:FireServer(Vector2.new((v[1] + 0.5), (v[2] + 0.5)), v[3], true) end)
                sent += 1
                if sent % 20 == 0 then twait() end
            end
            return sent
        end
        ...
        CoruTask.New("Fly", function()
            local vflyBV, vflyBG, vflyStateConn, vflyCharConn = nil, nil, nil, nil;
            local vflyRoot, vflyHum = nil, nil;

            local function refreshVFlyChar()
                selc = selff.Character or selc;
                HumSelf = selc and FindFirstChildOfClass(selc, "Humanoid") or nil;
                HumRSelf = selc and (FindFirstChild(selc, "HumanoidRootPart") or (HumSelf and HumSelf.RootPart)) or nil;
                Cam = GG.W.CurrentCamera or Cam;
                return selc, HumSelf, HumRSelf;
            end;

            local function destroyVFlyMovers()
                ...
                vflyBV, vflyBG = nil, nil;
            end;

            local function cleanVFly(resetHumanoid)
                pcall(function() if vflyStateConn then vflyStateConn:Disconnect(); vflyStateConn = nil; end end);
                destroyVFlyMovers();
                if resetHumanoid then
                    refreshVFlyChar();
                    local hum = HumSelf or vflyHum;
                    pcall(function()
                ...
                vflyRoot, vflyHum = nil, nil;
            end;

            local function applyVFly()
                cleanVFly(false);
                refreshVFlyChar();
                if not HumRSelf or not HumSelf then return false end;

                vflyRoot, vflyHum = HumRSelf, HumSelf;

                pcall(function()
                    local old = HumRSelf:FindFirstChild("FiHonVFlyGyro");
                    if old then old:Destroy(); end;
                    old = HumRSelf:FindFirstChild("FiHonVFlyVelocity");
                    if old then old:Destroy(); end;
                end);

                vflyBG = Instancen("BodyGyro");
                vflyBG.Name = "FiHonVFlyGyro";
                vflyBG.MaxTorque = Vec3(9e9, 9e9, 9e9);
                vflyBG.P = 9e4;
                vflyBG.D = 1e3;
                vflyBG.CFrame = HumRSelf.CFrame;
                vflyBG.Parent = HumRSelf;

                vflyBV = Instancen("BodyVelocity");
                vflyBV.Name = "FiHonVFlyVelocity";
                vflyBV.MaxForce = Vec3(9e9, 9e9, 9e9);
                vflyBV.P = 1e4;
                vflyBV.Velocity = Vector3.zero;
                vflyBV.Parent = HumRSelf;

                pcall(function()
                    HumSelf.WalkSpeed = 0;
                    HumSelf:ChangeState(Enum.HumanoidStateType.Physics);
                end);
                vflyStateConn = HumSelf.StateChanged:Connect(function(_, new)
                    if ClientCon["Enable Fly"] and new ~= Enum.HumanoidStateType.Physics then
                        tk.defer(function()
                            if ClientCon["Enable Fly"] and HumSelf and HumSelf.Parent then
                                HumSelf:ChangeState(Enum.HumanoidStateType.Physics);
                            end;
                        end);
                    end;
            ...
            vflyCharConn = selff.CharacterAdded:Connect(function()
                tk.spawn(function()
                    twait(0.7);
                    if ClientCon["Enable Fly"] and not CoreDestroyed then
                        applyVFly();
                    end;
                end);
            end);
            ...
            while true do
                if not ClientCon["Enable Fly"] or CoreDestroyed then
                    pcall(function() if vflyCharConn then vflyCharConn:Disconnect(); vflyCharConn = nil; end end);
                    cleanVFly(true);
                    CoruTask.Close("Fly");
                end;

                refreshVFlyChar();
                if not HumRSelf or not HumSelf then
                    twait(0.08);
                    continue;
                end;

                -- Re-attach after teleport/respawn/reset or if Roblox removes BodyMovers.
                if HumRSelf ~= vflyRoot or HumSelf ~= vflyHum or not vflyBV or not vflyBV.Parent or not vflyBG or not vflyBG.Parent then
                    applyVFly();
                end;

                if HumRSelf and HumSelf and Cam and vflyBV and vflyBG then
                    pcall(function()
                        if HumSelf:GetState() ~= Enum.HumanoidStateType.Physics then
                            HumSelf:ChangeState(Enum.HumanoidStateType.Physics);
                        end;
                        HumSelf.WalkSpeed = 0;
                    end);
                    ...
                    local flatL = Vec3(look.X, 0, look.Z);
                    local flatR = Vec3(right.X, 0, right.Z);
                    if flatL.Magnitude > 0 then flatL = flatL.Unit; end;
                    if flatR.Magnitude > 0 then flatR = flatR.Unit; end;

                    local move = Vector3.zero;
                    local md = HumSelf.MoveDirection;
                    ...
                    local speed = (tonumber(ClientCon.FlySpeed) or 1) * 50;
                    if move.Magnitude > 0.08 then
                        vflyBV.Velocity = move.Unit * speed;
                        if look.Magnitude > 0 then
                            vflyBG.CFrame = CFrame.new(HumRSelf.Position, HumRSelf.Position + look);
                        end;
                    else
                        vflyBV.Velocity = Vector3.zero;
                        vflyBG.CFrame = HumRSelf.CFrame;
            ...
            ClientTab = {
                {type="Group", dats={
                    {dat={
                        ...
                        {type="Slider", EN="VFly Speed", EN2="Change VFly speed.", TH1="ความเร็วในการบิน", TH2="ปรับความเร็ว VFly", Value={Min=1, Max=10} , Path="Client/FlySpeed"},
                    ...
                    {dat={
                        {type="Group", dats={
                            {dat={
                                ...
                                {type="Button", EN="Auto Blessing Ink", EN2="Collect Orchid/Water and place the blessing ink.", TH1="ออโต้หมึก", TH2="เก็บวัตถุดิบและวางหมึก", Callback=IB_NO_VIRTUALIZE(function()
                                    return Functions:B2C4Func("Draw/Ink");
                                end)};
        ...
        local LSecureUI = function()
            ...
            pcall(function() __InstallCompactNotify(WindUI, "FiHon") end)
            ...
            local openButtonScale = UserInputService.TouchEnabled and 0.72 or 0.66
            local DEFAULT_UI_SCALE = UserInputService.TouchEnabled and 0.62 or 0.60

            pcall(function()
                if ScriptCache.Window and ScriptCache.Window.Destroy then
                    ScriptCache.Window:Destroy()
                end
            end)

            local Window = WindUI:CreateWindow({
                Title = "FiHon - The Mimic",
                Icon = NEVOIRS_UI_ICON,
                Author = "Book 1 + Book 2 + Client",
                Folder = "FiHon_The_Mimic",
                Size = windowSize,
                MinSize = minSize,
                MaxSize = maxSize,
            ...
            if not Window then
                __errorGui("WindUI nil", "CreateWindow trả về nil. Hãy thoát menu WindUI cũ rồi execute lại.")
                return
            end
            ...
            local function styleAvatarImage(obj)
            ...
            local function applyAvatarChrome()
                local playerGui = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
                if not playerGui then return end
                for _, obj in ipairs(playerGui:GetDescendants()) do
                    pcall(styleAvatarImage, obj)
                end
            end

            applyAvatarChrome()
            task.delay(0.25, applyAvatarChrome)
            task.delay(0.8, applyAvatarChrome)
            pcall(function()
                local playerGui = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
                if playerGui then
            ...
            local function toggleFiHonWindow()
                local ok = false
                pcall(function()
            ...
            local function createFiHonLeftToggle()
                local pg = LocalPlayer and LocalPlayer:FindFirstChildOfClass("PlayerGui")
                if not pg then return end
                local old = pg:FindFirstChild("FiHon_LeftToggle_UI")
                if old then old:Destroy() end

                local gui = Instance.new("ScreenGui")
                gui.Name = "FiHon_LeftToggle_UI"
                gui.ResetOnSpawn = false
                gui.IgnoreGuiInset = true
                gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
                ...
                btn.MouseButton1Click:Connect(toggleFiHonWindow)
            end

            createFiHonLeftToggle()

            pcall(function()
                WindUI:Notify({
                    Title = "FiHon - The Mimic",
                    Content = "Đã tải UI. RightCtrl hoặc nút tròn bên trái để ẩn/hiện menu.",
                    Icon = "check",
                    Duration = 2,
                })
            end)
            ...
            local B1C1Tab = Window:Tab({ Title = "B1C1", Icon = "book-open" })
            local B1C2Tab = Window:Tab({ Title = "B1C2", Icon = "book-open" })
            local B1C3Tab = Window:Tab({ Title = "B1C3", Icon = "book-open" })
            local B1C4Tab = Window:Tab({ Title = "B1C4", Icon = "book-open" })
            pcall(function() Window:Divider() end)
            local B2C1Tab = Window:Tab({ Title = "B2C1", Icon = "book-open" })
            ...
            local ChristmasTrialTab = Window:Tab({ Title = "Christmas Trial", Icon = "snowflake" })
            local HalloweenTrialTab = Window:Tab({ Title = "Halloween Trial", Icon = "ghost" })
            local NightmareCircusTab = Window:Tab({ Title = "Nightmare Circus", Icon = "party-popper" })


            pcall(function()
                if SettingTab and SettingTab.Paragraph then
                    SettingTab:Paragraph({
                        Title = "UI Scale",
                        Desc = "Chỉ chỉnh kích cỡ tổng thể UI. Mặc định đang để thấp để đỡ chèn ép màn hình.",
                        Image = "settings",
                        ImageSize = 14,
                        Color = Color3.fromRGB(78, 158, 230),
                    })
            ...
            pcall(function()
                if SettingTab and SettingTab.Slider then
                    SettingTab:Slider({
                        Title = "UI Scale",
                        Step = 0.01,
                        IsTextbox = true,
                        IsTooltip = false,
                        Width = UserInputService.TouchEnabled and 130 or 150,
                        Value = { Min = 0.58, Max = 0.90, Default = DEFAULT_UI_SCALE },
                        Callback = function(value)
                            value = tonumber(value) or DEFAULT_UI_SCALE
                            value = math.clamp(value, 0.58, 0.90)
            ...
            local function build(tab, data, scope)
                if not tab or not data then return end
                local ok, err = pcall(function()
                    Windy:CreateComponent(tab, data, scope)
                end)
                if not ok then __notify("Build tab lỗi: " .. tostring(scope), tostring(err), "circle-alert", 6) end
            end
            ...
            local function buildSections(tab, data, scope)
                if not LoaderSettings.TheMimicLoader.Load_Sections then return end
                local ok, err = pcall(function()
                    Functions.PairingSections(tab, data, scope)
                end)
                if not ok then __notify("Sections lỗi: " .. tostring(scope), tostring(err), "circle-alert", 6) end
            end

            build(B1C1Tab, ScriptData.AutoData.B1C1Tab, "B1C1")
            buildSections(B1C1Tab, ScriptData.Sections.B1C1, "B1C1")
            build(B1C2Tab, ScriptData.AutoData.B1C2Tab, "B1C2")
            buildSections(B1C2Tab, ScriptData.Sections.B1C2, "B1C2")
            build(B1C3Tab, ScriptData.AutoData.B1C2Tab, "B1C3")
            buildSections(B1C3Tab, ScriptData.Sections.B1C3, "B1C3")
            build(B1C4Tab, ScriptData.AutoData.B1C4Tab, "B1C4")

            build(B2C1Tab, ScriptData.AutoData.B2C1Tab, "B2C1")
            ...
            build(HalloweenTrialTab, ScriptData.AutoData.HalloweenTrialTab, "Halloween")
            build(NightmareCircusTab, ScriptData.AutoData.NightmareCircusTab, "NightmareCircus")

            pcall(function() ClientTab:Select() end)
            pcall(function()
            ...
            local OneRunCallMain, OneRunErrorMain = pcall(function()
                ...
                CoreConnection[1] = GG.H.Stepped:Connect(IB_NO_VIRTUALIZE(function()
                    ...
                    HumRSelf = HumSelf and (HumSelf.RootPart or FindFirstChild(selc, "HumanoidRootPart"));
                ...
                if not ScriptCache.AlreadyFiHonFlyInput then
                    ScriptCache.AlreadyFiHonFlyInput = true;
                    UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
                        if gameProcessedEvent then return; end;
                        Functions.SetFlyKey(input, true);
                    end);
                    UserInputService.InputEnded:Connect(function(input, gameProcessedEvent)
                        Functions.SetFlyKey(input, false);
                    end);
                end;
        ...
        end; GG.LSecureLoad = LSecureLoad;

        local __FiHonLoadOK, __FiHonLoadErr = pcall(function()
            return LSecureLoad("STANDALONE")
        end);
        if not __FiHonLoadOK then
            __errorGui("FiHon load failed", tostring(__FiHonLoadErr));
        end;